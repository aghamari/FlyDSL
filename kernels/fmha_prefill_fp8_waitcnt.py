...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_waitcnt_smem")
...
# F4 LEVER — PARTIAL LDS drain: s_waitcnt lgkmcnt(N), wait until <= N LGKM (LDS/scalar)
# ops remain outstanding instead of fully draining (lgkmcnt(0)). Pairs the wait with the
# downstream consumer so the MFMA/VALU stream overlaps the tail of the ds_bpermute latency,
# mirroring CK-Tile / PyISA which use a partial lgkmcnt(N) rather than a full drain.
#
# gfx942 (CDNA3) s_waitcnt SIMM16 field layout, DERIVED from the two existing constants
# (NOT guessed) — decode _LGKMCNT0 = 0xC07F = 0b1100_0000_0111_1111 and _VMCNT0 = 0x3F70:
#   vmcnt[3:0]  = bits[3:0]        (0xC07F -> 1111, _VMCNT0 -> 0000 == vmcnt 0)
#   expcnt[2:0] = bits[6:4]        (both -> 111 == don't-care)
#   lgkmcnt[3:0]= bits[11:8]       (0xC07F -> 0000 == lgkmcnt(0); _VMCNT0 -> 1111 don't-care)
#   vmcnt[5:4]  = bits[15:14]      (0xC07F -> 11 don't-care; _VMCNT0 -> 00 == vmcnt 0)
# _LGKMCNT0 already carries vmcnt/expcnt = all-ones (don't-care) with lgkmcnt = 0, so setting
# lgkmcnt(N) is just OR-ing N into bits[11:8]:  imm = 0xC07F | ((N & 0xF) << 8).
#   lgkmcnt(0) -> 0xC07F (== _LGKMCNT0, self-check)   lgkmcnt(1) -> 0xC17F
#   lgkmcnt(2) -> 0xC27F                              lgkmcnt(3) -> 0xC37F
def _wait_lds_n(n):
    fx.rocdl.s_waitcnt(_LGKMCNT0 | ((n & 0xF) << 8))
        ...
        def compute_kt_tile(kv0_outer, kbuf, vbuf, m_run, l_run, o_acc, do_mask):
            ...
            for sub in fx.range_constexpr(NSUB):
                ...
                for s in fx.range_constexpr(2):
                    ...
                    # F4 LEVER (THE ONE CHANGE) — split the single full LDS drain into a partial +
                    # full pair so w0's v_cndmask issues while the last two bpermutes still drain.
                    # Issue order of the 4 same-latency ds_bpermute is h0_b0,h0_b1,h1_b0,h1_b1; CDNA3
                    # decrements lgkmcnt for identical in-flight DS ops in that order. w0 consumes only
                    # the FIRST two (h0_b0,h0_b1) -> lgkmcnt(2) guarantees exactly those are landed,
                    # while h1_b0/h1_b1 keep draining overlapped with the v_cndmask + i64 pack.
                    _wait_lds_n(2)
                    w0 = is_h0.select(h0_b0, h0_b1)
                    # w1 consumes the LAST two (h1_b0,h1_b1); a full drain here guarantees they are
                    # landed before w1 is packed into p_i64 and read by GEMM2 -> no stale P.
                    _wait_lds()
