...
★ hk6 = hk5 + three ATT-driven, VGPR-neutral levers stacked (all measured device-fair on MI308X,
graph-replay, vs hk5 124/143 TF @ sq16384/32768):
  1. SCORE-VALU FOLD: per-kv descale `s = sv*(qs*kdv)` folded so `qs` is applied once via an
     exp2 FMA (`exp2(fma(qs, u, -safe_m_p))`) on the unmasked path — removes ~30 v_mul_f32/tile.
  2. PREFETCH RELOCATE: the OPT3 next-tile K/V prefetch (`load_kv_regs`) is issued AFTER GEMM1
     (overlapping the softmax VALU) instead of at loop top, so the P-transpose `_wait_lds()` no
     longer stalls on the prefetch drain — ATT VMEM-wait 24.3M -> 13.7M cycles.
  3. O-RESCALE SKIP: the online O-accumulator rescale `o_acc *= corr` (64 muls/tile) is skipped on
     tiles where the running max did not grow (wave-uniform ballot on `m_new > m_run`; corr==1 there,
     so skipping is EXACT). Biggest single win — causal max stabilizes, so most interior tiles skip.
Result: **21 / 54 / 140 / 160 TF** @ sq 1024/2048/16384/32768 (CK-Tile fp8: 30/62/141/146) — beats
CK at sq32768, ties at sq16384. Still 3 waves/SIMD (166 VGPR); the 4-wave occupancy path stays
blocked (half-O peak is softmax-temps; maxnreg broken in the wheel). Correctness err 0.039-0.049 OK.
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_ck_hk6_smem")
    ...
    def process_qtile(qtile):
        ...
        def compute_kt_tile(kv0_outer, kbuf, vbuf, m_run, l_run, o_acc, do_mask):
            ...
            for sub in fx.range_constexpr(NSUB):
                ...
                for i in fx.range_constexpr(16):
                    if const_expr(do_mask):
                        ...
                        # SVALU fold: keep the per-element descale a SINGLE mul by dropping the
                        # `qs *` factor here. qs (= q_descale*sm_scale*LOG2E, > 0, constant per
                        # q-row/lane) is deferred into the exp2 FMA below. The score is carried in
                        # "u-space" (u == s / qs) through the max reduction; since qs > 0 and is
                        # uniform across the elements/lanes being max-reduced, max(qs*u) == qs*max(u),
                        # so we reapply qs once after the reduction. Saves 16 VALU muls per subtile.
                        s = sv[sub][i] * kdv[i // 4][i % 4]
                    s_sub.append(fx.Float32(s))
                s_all.append(s_sub)

            # OPT3 prefetch (lever I2, relocated): issue the NEXT KT tile's global loads here — at
            # the head of the softmax, after GEMM1 and after the rkd descale loads above have been
            # consumed — instead of at the top of loop_body. The max-reduce + 16 exp2 VALU that
            # follow give the loads independent work to hide behind, and they no longer stay
            # outstanding across the P-transpose _wait_lds(). Kept AFTER the descale loads so their
            # vmcnt wait does not drag the prefetch in early. Handed back to loop_body for
            # store_kv_to_lds: the load still precedes its store and the addresses/values are the
            # same as the base, so the result is numerically identical.
            ...
            if const_expr(not do_mask):
                # Lift the u-space tile max back into s-space with the single deferred qs mul.
                # qs is identical across the shuffle_xor lane-pair (same q_local -> same q_descale)
                # and across the 16 tile elements, so this equals the baseline max over qs*u.
                m_loc = m_loc * qs
            ...
            for sub in fx.range_constexpr(NSUB):
                ...
                for i in fx.range_constexpr(16):
                    if const_expr(do_mask):
                        ...
                        # descale mul and the max-subtract into ONE VALU op (vs the baseline's
                        # separate mul + sub), keeping exp2 itself unchanged.
            ...
            # P1 (o-rescale-skip): wave-uniform skip of the O-accumulator rescale.
            # corr == exp2(m_run - safe_m) is EXACTLY 1.0 on any lane whose running max did NOT grow
            # this tile (m_new == m_run), so `o_acc *= corr` is a no-op there. The O rescale is a
            # wave-wide op while corr is per-lane, so we may only skip it when NO active lane in the
            # wave grew its max (safe superset: if ANY lane grew, rescale ALL lanes -> still exact).
            # The per-lane `changed` predicate is collapsed to a wave-uniform SGPR condition with a
            # single 64-lane ballot, so the skip lowers to a scalar s_cbranch that elides the whole
            # DT*16 = 64 f32-mul rescale AND its serial max-reduce -> corr(exp2) -> rescale stall
            # (the #1 VALU/"other" hotspot at sq16384). The max / l_loc / l_run bookkeeping above is
            # left byte-identical and is NEVER skipped -- only the O multiply is made conditional.
            changed = m_new > m_run  # per-lane: this lane raised its running max (=> its corr < 1)
            bal = fx.Int64(fx.rocdl.ballot(fx.typing.T.i64, _ar(changed)))
            any_changed = bal != fx.Int64(0)  # wave-uniform: did ANY active lane grow its max?
            # DT == 4 here (HD == 128); carry the O fragments as named scalars so the runtime `if`
            # yields them through scf.if (a Python list cannot be an scf.if result value).
            o0 = o_acc[0]
            o1 = o_acc[1]
            o2 = o_acc[2]
            o3 = o_acc[3]
        ...
        def loop_body(kt_iv, m_run, l_run, o_acc, do_mask):
            ...
            # OPT3 prefetch is now issued INSIDE compute_kt_tile (head of softmax), not here.
