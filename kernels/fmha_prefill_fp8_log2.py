...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_log2_smem")
...
def attn_kernel(
    ...
    for kt_iv, st in range(fx.Index(0), fx.Index(n_kv_rt), fx.Index(1), init=init_state):
        ...
        for i in fx.range_constexpr(16):
            ...
            # VALU LEVER (log2 domain): fold LOG2E into the score scale so s is already in
            # log2 units -> the exp args below drop their per-element *LOG2E (16 muls/tile +
            # the corr mul saved). Max/mask are monotonic, identical in log2 domain.
            s = sv[i] * (q_descale * k_descale * fx.Float32(sm_scale * LOG2E))
        ...
        _ar = fx.arith.unwrap
        # log2 domain: m_run/safe_m are already log2-scaled, so corr = 2^(m_run - safe_m), no *LOG2E.
        corr = fx.Float32(fx.rocdl.exp2(f32t, _ar(m_run - safe_m)))
        corr = m_is_neg.select(fx.Float32(0.0), corr)

        # p = 2^(s - m_new) (s already log2-scaled); l_tile = sum p (cross-half)
        l_loc = fx.Float32(0.0)
        p_vals = []
        for i in fx.range_constexpr(16):
            p = fx.Float32(fx.rocdl.exp2(f32t, _ar((s_vals[i] - safe_m) + log2_pscale)))
            p_vals.append(p)
            l_loc = l_loc + p
