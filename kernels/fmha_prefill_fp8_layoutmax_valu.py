...
"""FP8 causal FMHA prefill — layoutmax + VECTORIZED score-scaling (VALU lever, runbook §7.4/§8.5).

Builds on ``fmha_prefill_fp8_layoutmax`` (the maximally layout-algebraic hk5). Only added change:
the per-kv descale score-scaling ``S = sv * qs * kdv`` (run on EVERY kv tile) is done as PACKED
vector muls (``sv * (kdv16 * qs)``) instead of 16 scalar f32 muls per subtile, folding into
``v_pk_mul_f32``. Measured +9% @ sq32768 on the hk5 lineage; correctness byte-identical.
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_layoutmax_valu_smem")
