...
"""FP8 causal FMHA prefill (paged, vec_k_col_v) — LAYOUT-ALGEBRAIC-MMA variant of hk6.

★ Learning artifact + honest scope. The layout algebra here expresses the TWO MATMULS ONLY:
``make_mma_atom(MFMA 32x32x16 fp8)`` -> ``make_tiled_mma`` -> per-lane ``thr_slice`` -> typed
``make_fragment_A/B/C`` -> ``fx.gemm`` (no raw ``mfma_*`` calls, no manual i64 operand packing).
Note even here the operand DATA is hand-loaded and ``.store()``-d into the fragments; the algebra
supplies the fragment SHAPES and drives the matmul.

The GRID DECODE is layout algebra: block-id <-> (qhead, first_idx, batch) is a coordinate<->index
mapping = ``idx2crd(blk, layout)`` (verified == the manual div/mod; zero perf change since it lowers
to the same ops). This is the one place beyond the MMA where the algebra genuinely fits.

The DATA MOVEMENT, however, is necessarily MANUAL index arithmetic — this is NOT "maximally
layout-algebraic" in the tiled-copy/partition sense, because FMHA's accesses don't fit the affine
tiled-copy recipe: the paged KV gather (LTD/LTP page-table indirection), the custom padded /
column-V LDS staging, the ``ds_bpermute`` cross-lane P-transpose, per-kv descale, and the transposed
[d,q]->[q,d] epilogue scatter are all hand-computed byte offsets. The XCD chiplet remap is a layout
composition in principle, but ``crd2idx`` won't take an ``idx2crd`` result directly in this wheel, so
its permutation stays manual (it's an optional perf lever regardless). (Attempted the causal-mask via ``partition_C`` +
``make_identity_layout`` coord tensor — the coord orientation matches the MFMA C layout in isolation
but did not integrate correctly here; see FINDINGS.md. For a kernel whose data movement DOES fit the
full algebra, see ``examples/efficient-gemm.py`` — a dense GEMM.)

= ``fmha_prefill_fp8_layoutmax`` (layout-algebraic-MMA hk5) + hk6's three ATT-driven levers stacked
on the softmax/loop (the GEMM path is untouched):
  1. SVALU fold: per-kv descale applied once via an exp2 FMA (``exp2(fma(qs, u, -safe_m_p))``) on
     the unmasked path — scores carried in u-space through the max, qs reapplied once after.
  2. PREFETCH relocate: the OPT3 next-tile K/V prefetch is issued at the head of the softmax
     (after GEMM1) so it overlaps the softmax VALU instead of stalling the P-transpose wait.
  3. O-RESCALE skip: wave-uniform ``ballot(m_new > m_run)`` elides the 64-mul online O rescale on
     tiles where the causal running-max did not grow (corr == 1 there -> exact).

BOTH matmuls go through the layout-algebra MMA path: one typed ``make_mma_atom(MFMA 32x32x16 fp8)``
-> ``make_tiled_mma`` -> per-lane ``thr_slice`` whose derived A/B/C fragments drive ``fx.gemm`` (no
raw ``mfma_f32_32x32x16_fp8_fp8`` calls, no manual i64 operand packing). Q/K/V/P operands are fed
as typed ``make_fragment_A/B/C`` tiles.

What stays necessarily manual (no layout-algebra expression exists for it): the PAGED KV gather
(LTD/LTP page-table indirection + per-page strides), the fp8 ``cvt_pk`` packing, the ``ds_bpermute``
P-transpose, per-kv-group descale, and the transposed [d,q]->[q,d] epilogue scatter.

★ See the canonical ``fmha_prefill_fp8_ck_hk5`` for full design notes; perf history, the
VALU-fold/COLUMN-V wins, occupancy diagnosis, and the dead-end ledger live in
``learn_fmha/docs/JOURNAL.md`` §2–3.
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_layoutmax_hk6_smem")
...
def attn_kernel(
    ...
    # LAYOUT-ALGEBRA COORDINATE MAPPING: block-id <-> (qhead, first_idx, batch) is exactly what a
    # Layout is (a coordinate<->index function), so the grid decode is idx2crd, and the XCD remap is
    # a layout COMPOSITION (decompose blk on one layout, recompose on the reordered one). The grid is
    # launched as b*nq*num_first blocks, so the batch extent B = gdim / (nq*num_first).
    _nq = fx.Int32(nq)
    gdim = fx.Int32(fx.grid_dim.x)
    B = gdim // (_nq * num_first)
    if const_expr(XCD_REMAP):
        # XCD chiplet remap as a layout composition: blk decodes on (NXCD, XCD_C, nchunks) with the
        # HW-round-robin strides (1, NXCD, chunk_span) -> (xcd, pos, chunk); recompose on the SAME
        # shape with reordered strides (XCD_C, 1, chunk_span) so XCD_C consecutive logical ids land
        # on one XCD. Only the FULL-chunk prefix is permuted; the partial tail keeps identity (guard
        # is a runtime boundary, not expressible in the algebra) -> the whole map stays a bijection.
        # (The permutation core is a layout composition in principle, but crd2idx does not accept an
        # idx2crd IntTuple result directly in this wheel, so it stays manual index math. It is an
        # optional perf lever anyway; the DECODE below is the clean layout-algebra win.)
        ...
        # Decode QHEAD-FAST: lblk = coord (qhead, first_idx, batch) through (nq, num_first, B) with
        # strides (1, nq, nq*num_first). idx2crd IS the decode (== the qhead/first_idx/batch div/mod).
        _crd = fx.idx2crd(lblk, fx.make_layout((_nq, num_first, B), (fx.Int32(1), _nq, _nq * num_first)))
        qhead = fx.Int32(fx.get(_crd, 0))
        first_idx = fx.Int32(fx.get(_crd, 1))
        batch = fx.Int32(fx.get(_crd, 2))
    else:
        # Decode FIRST_IDX-FAST: blk = coord (first_idx, qhead, batch) through (num_first, nq, B)
        # with strides (1, num_first, num_first*nq).
        _crd = fx.idx2crd(blk, fx.make_layout((num_first, _nq, B), (fx.Int32(1), num_first, num_first * _nq)))
        first_idx = fx.Int32(fx.get(_crd, 0))
        qhead = fx.Int32(fx.get(_crd, 1))
        batch = fx.Int32(fx.get(_crd, 2))
    ...
    # MAXIMALLY LAYOUT-ALGEBRAIC MMA: one typed 32x32x16 fp8 atom -> make_tiled_mma, whose derived
    # A/B/C fragment layouts drive fx.gemm for BOTH GEMMs (replacing the raw mfma_f32_32x32x16 calls).
    # Shape-only fragment donors (K is paged 5-D, O is 4-D -> take flat (32,16)/(32,32) views).
    ...
    def process_qtile(qtile):
        ...
        # Q LOADED ONCE into registers (CK kQLoadOnce), reused over the whole KV loop. Held as fp8x8
        # GEMM1-B fragment operands (fed straight to fx.gemm; no manual i64 bitcast).
        q_frag = []
        ...
        def compute_kt_tile(kv0_outer, kbuf, vbuf, m_run, l_run, o_acc, do_mask):
            ...
            for sub in fx.range_constexpr(NSUB):
                ...
                for i in fx.range_constexpr(16):
                    if const_expr(do_mask):
                        ...
                        # SVALU fold (lever 1): keep the descale a SINGLE mul by dropping `qs *`.
                        # qs (= q_descale*sm_scale*LOG2E, >0, per-lane constant) is deferred into the
                        # exp2 FMA below; the score is carried in "u-space" (u == s/qs) through the
                        # max, and since qs>0 is uniform over the reduced elements/lanes,
                        # max(qs*u) == qs*max(u), so qs is reapplied once after the reduction.
                        s = sv[sub][i] * kdv[i // 4][i % 4]
                    s_sub.append(fx.Float32(s))
                s_all.append(s_sub)

            # OPT3 prefetch (lever 2, relocated): issue the NEXT KT tile's global loads here — at the
            # head of the softmax, after GEMM1 + after the rkd descale loads above — so the max-reduce
            # + 16 exp2 VALU hide their latency and they no longer stay outstanding across the
            # P-transpose _wait_lds(). Handed back to loop_body for store_kv_to_lds; load still
            # precedes its store with identical addresses/values, so the result is numerically same.
            ...
            if const_expr(not do_mask):
                # Lift the u-space tile max back to s-space with the single deferred qs mul (qs is
                # identical across the shuffle_xor lane-pair and the 16 elements, so this == the
                # baseline max over qs*u).
                m_loc = m_loc * qs
            ...
            if const_expr(not do_mask):
                neg_safe_m_p = log2_pscale - safe_m  # == -safe_m_p, the u-space exp FMA addend
            ...
            for sub in fx.range_constexpr(NSUB):
                ...
                for i in fx.range_constexpr(16):
                    if const_expr(do_mask):
                        ...
                        # exp2 arg = qs*u - safe_m_p == fma(qs, u, -safe_m_p): fuses the deferred
                        # descale mul and the max-subtract into ONE VALU op.
                        arg = fx.math.fma(qs, s_all[sub][i], neg_safe_m_p)
                        p = fx.Float32(fx.rocdl.exp2(f32t, _ar(arg)))
                    p_sub.append(p)
                    l_loc = l_loc + p
                p_all.append(p_sub)
            l_loc = l_loc + l_loc.shuffle_xor(off32, width64)
            l_run = l_run * corr + l_loc
            # O-RESCALE skip (lever 3): corr == exp2(m_run - safe_m) is EXACTLY 1.0 on any lane whose
            # running max did NOT grow this tile, so `o_acc *= corr` is a no-op there. The rescale is
            # wave-wide while corr is per-lane, so skip only when NO active lane grew (safe superset:
            # if ANY grew, rescale ALL -> still exact). A single 64-lane ballot collapses the per-lane
            # predicate to a wave-uniform SGPR condition -> the whole DT*16=64 f32-mul rescale (and its
            # serial max->exp2->rescale stall, the #1 VALU hotspot) lowers to a scalar s_cbranch skip.
            # max/l_loc/l_run bookkeeping is NEVER skipped -- only the O multiply is conditional.
            changed = m_new > m_run
            bal = fx.Int64(fx.rocdl.ballot(fx.typing.T.i64, _ar(changed)))
            any_changed = bal != fx.Int64(0)
            # DT == 4 (HD == 128); carry O fragments as named scalars so the runtime `if` can yield
            # them through scf.if (a Python list cannot be an scf.if result value).
            o0 = o_acc[0]
            o1 = o_acc[1]
            o2 = o_acc[2]
            o3 = o_acc[3]
            if any_changed:
                ...
                o0 = fx.Vector(o0) * corr_vec
                o1 = fx.Vector(o1) * corr_vec
                o2 = fx.Vector(o2) * corr_vec
                o3 = fx.Vector(o3) * corr_vec
            o_acc = [o0, o1, o2, o3]
            ...
            for sub in fx.range_constexpr(NSUB):
                ...
                for dt in fx.range_constexpr(DT):
                    ...
                    for s in fx.range_constexpr(2):
                        ...
                        v_packs.append(fx.Vector(vv8))
                for dt in fx.range_constexpr(DT):
                    frag_O.store(fx.Vector(o_acc[dt]))
                    for s in fx.range_constexpr(2):
                        frag_A2.store(fx.Vector(v_packs[dt * 2 + s]).bitcast(fx.Float8E4M3FNUZ))
            ...
            return m_new, l_run, o_acc, kc_w_next, vc_w_next

        # One outer-tile step. Prefetch (lever 2) now issued INSIDE compute_kt_tile and returned.
        def loop_body(kt_iv, m_run, l_run, o_acc, do_mask):
            ...
            m_run, l_run, o_acc, kc_w_next, vc_w_next = compute_kt_tile(
                kv0_outer, kbuf, vbuf, m_run, l_run, o_acc, do_mask
            )
...
# Per-seqlen (KT, DIAG) dispatch — same measured optima as hk6.get_kernel (this kernel matches
# hk6 device-fair: 28/60/120/143/160 TF @ sq 1024/2048/8192/16384/32768). One (KT,DIAG) per
# process (SmemAllocator finalizes once); get_kernel(sq) sets the env then re-imports fresh.
