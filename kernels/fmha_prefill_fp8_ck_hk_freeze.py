...
"""FP8 causal FMHA prefill (paged, vec_k_col_v) for gfx942 — hk5 + Schraudolph fast-exp2.

VARIANT of ``fmha_prefill_fp8_ck_hk5.py`` (the canonical kernel). The ONLY algorithmic change
is the softmax P=exp: the per-element quarter-rate ``v_exp_f32`` is replaced by the affine
Schraudolph 2^x bit-trick ``P = bitcast_f32(u32(round(x*AC + bias)))`` (full-rate v_fma + a
single v_cvt_u32_f32), ported from the MI350 PyISA kernel's ``_USE_EXP2_APPROX`` (lines 61-65,
666-678). The running-max + corr (rescale) path is kept EXACT. Gated by FMHA_FEXP:
  0 = exact rocdl.exp2 (== hk5, accuracy SKU);
  1 = pure affine (fastest, +16/+20% but fails the fp8 6e-2 gate -> lossy/fast SKU);
  2 = affine + quadratic mantissa correction (DEFAULT; +3.7/+6.2% AND passes the gate).

The u32 (not i32) convert is load-bearing: it floors the negative-overflow tail (x below
~-127) and masked -inf lanes to +0.0 (NaN-safe), exactly matching exp's underflow behaviour.

Tunables (env): FMHA_FEXP (fast-exp2 mode 0/1/2, default 2), plus all hk5 knobs: FMHA_NWAVES
(default 4 -> TILE_BM=128), FMHA_KT (32), FMHA_VCOL (1), FMHA_DIAG (1), FMHA_XCD (1; FMHA_XCD_C
4), FMHA_BUFK (0 -- broken in this wheel).
"""
...
# --- Schraudolph fast-exp2 (this variant's one lever) -------------------------------------
# Replace the per-element softmax exp2 with the affine bit-trick 2^x ~= bitcast(u32(round(
# x*AC + bias))). AC = 2^23 places the integer part into the f32 exponent field; BIAS folds in
# the +127 exponent offset and the Schraudolph error-minimising correction (127*2^23 - 486411 =
# 1064866805). x = s_all - safe_m_p (the same arg hk5 feeds rocdl.exp2); the subtract is folded
# into a per-tile effective bias so only ONE extra op (the cvt) is added vs the exact path.
#
# FMHA_FEXP modes:
#   0 = exact rocdl.exp2 (byte-identical to hk5; the accuracy SKU).
#   1 = pure affine Schraudolph (fastest, +16/+20% @ sq16384/32768; but ~3% peak rel-err ->
#       0.078-0.094 max-abs EXCEEDS the fp8 6e-2 gate, so it is a lossy/fast-only SKU, like the
#       MI350 _USE_EXP2_APPROX video default).
#   2 = affine + one quadratic mantissa correction (cvt-based, no transcendental/rcp/floor, keeps
#       the negative-tail->+0.0 floor); +3.7/+6.2% and MEETS the gate (0.039-0.047). DEFAULT.
# Default is 2 (the gate-passing fast path); a default that fails correctness would be unusable.
...
# Mode-2 correction: P_true = P_lin * r(frac), frac = mantissa(bits)/2^23 in [0,1). r is the fixed
# ratio 2^(frac+0.058)/(1+frac) (range ~[0.981,1.041]); a symmetric quadratic through frac={0,.5,1}
# fits it to ~0.6% residual. Coeffs are folded for fm = float(mantissa_bits) in [0,2^23):
#   r = C0 + A1*fm + A2*fm^2  (Horner: fma(fma(fm,A2,A1),fm,C0)).
_EXP2_C0 = 1.041030
_EXP2_A1 = -0.23832 / _EXP2_AC
_EXP2_A2 = 0.23832 / (_EXP2_AC * _EXP2_AC)
_EXP2_MANT = 0x7FFFFF

# --- Phase-2 PROBE: max-freezing on the unmasked interior loop -----------------------------
# FMHA_FREEZE=1 freezes the running max on the unmasked interior tiles: skip the per-tile rowmax
# reduction (the v_max3 chain + cross-lane shuffle) AND the o_acc/l_run corr rescale,
# exponentiating against a frozen seed pivot. The masked diagonal loop stays EXACT.
#
# MEASURED (MI308X, sq16384/32768, on top of FEXP=2):
#   * SPEED CEILING is compelling: 122/149 TF (+12/+15% vs hk5 baseline 109/129, +8/+9% vs the
#     FEXP=2 default 113/137). Notably FEXP mode stops mattering for speed (122/149 with both
#     FEXP=0 and FEXP=2) -- i.e. removing the rowmax-reduction LATENCY chain (which sits between
#     the two MFMA bursts) is what unblocks the schedule, and once it's gone even exact exp hides.
#   * But it is NOT correct without the rollback. This is an UPPER-BOUND PROBE: there is NO
#     overflow detect-after-exp + rollback (the MI350 _USE_ROLLBACK machinery), so:
#       - fixed pivot too high (16.0) -> all P underflow the fp8 P-pack -> ERR 2.98;
#       - even a near-true-max pivot (4.0) -> ERR 0.14-0.19 (still FAILs the 6e-2 gate), because
#         freezing VIOLATES the online-softmax invariant: when a later interior tile's score
#         exceeds the pivot, the algorithm must rescale earlier o_acc/l_run DOWN; freezing does
#         not, so earlier contributions are over-weighted. The rollback is therefore MANDATORY
#         (reconstruct delta=1/max(P) from P on the rare tile whose max exceeds the pivot, rescale
#         o_acc+l_run, no gemm_QK re-run) -- the documented "hard part", plus a correct per-row
#         seed (in hk5 the row-max lives in the LAST/diagonal tiles, processed last, so seeding the
#         interior freeze needs a loop reorder). Left as a measured, promising TODO; default OFF.
_FREEZE = int(os.environ.get("FMHA_FREEZE", "0")) != 0
_FREEZE_SEED = 4.0  # probe-only log2 pivot (moot when FREEZE off)

_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_ck_hk_freeze_smem")
...
def attn_kernel(
    ...
    def process_qtile(qtile):
        ...
        def compute_kt_tile(kv0_outer, kbuf, vbuf, m_run, l_run, o_acc, do_mask, freeze=False):
            ...
            # FREEZE PROBE: on the unmasked interior tiles, skip the per-tile rowmax reduction
            # (the v_max3 chain + cross-lane shuffle) AND the corr rescale -- pivot against the
            # frozen running max. corr == 1, so o_acc/l_run never rescale. NO overflow guard.
            if const_expr(freeze):
                m_new = m_run
                safe_m = m_run  # seeded to _FREEZE_SEED so the frozen exp doesn't overflow
                corr = fx.Float32(1.0)
            ...
            #
            # FAST-EXP2 (the lever): 2^(s - safe_m_p) ~= bitcast(u32(round(s*AC + bias_eff))),
            # bias_eff = BIAS - safe_m_p*AC hoisted ONCE per tile (one fma) so the per-element
            # cost is exactly fma(s,AC,bias_eff) + cvt_u32 (the bitcast is free). The exact path
            # is sub + quarter-rate exp2; this swaps the exp2 for a full-rate cvt. The u32 cvt
            # saturates negative/-inf (masked) lanes to +0.0 -> matches exp's underflow floor.
            ...
            for sub in fx.range_constexpr(NSUB):
                ...
                for i in fx.range_constexpr(16):
                    if const_expr(_FEXP != 0):
                        ...
                        if const_expr(_FEXP == 2):
                            # Quadratic mantissa correction: P *= r(frac). Saturated n=0 (negative
                            # tail / masked -inf) -> fm=0, r=C0, p_lin=0 -> P=0 stays floored.
            ...
            if const_expr(freeze):
                # corr == 1: no o_acc/l_run rescale (the whole point of the freeze).
                l_run = l_run + l_loc
            else:
        ...
        # FREEZE: seed the running max to a generous constant so the frozen exp doesn't overflow
        # from the -inf init (the real rollback would seed from tile 0 / the masked diagonal).
        m_run0_p1 = fx.Float32(_FREEZE_SEED) if const_expr(_FREEZE) else m_run0
        init_state = [m_run0_p1, l_run0] + o_acc0
        for kt_iv, st in range(fx.Index(0), fx.Index(n_unmask), fx.Index(1), init=init_state):
            m_run = st[0]
            l_run = st[1]
            o_acc = [st[2 + d] for d in range(DT)]
            m_run, l_run, o_acc = loop_body(kt_iv, m_run, l_run, o_acc, False, _FREEZE)
