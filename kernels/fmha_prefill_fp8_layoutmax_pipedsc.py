...
"""FP8 causal FMHA prefill — layoutmax + packed score-scale + wait-batch + PIPELINE + descale-decouple.

Lineage: ``layoutmax`` -> ``_valu`` (packed score-scale) -> ``_valupt`` (P-transpose wait-batch)
-> ``_pipe`` (cross-tile GEMM1 pipeline) -> THIS (+ descale-decouple). Four stacked levers over the
maximally layout-algebraic hk5:
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_layoutmax_pipedsc_smem")
...
def attn_kernel(
    ...
    def process_qtile(qtile):
        ...
        def load_descale(kv0_outer, do_mask):
            kdv16_all = []
            for sub in fx.range_constexpr(NSUB):
        ...
        def _pack(m_run, l_run, o_acc, sv, kc, vc):
            return ([m_run, l_run] + [o_acc[d] for d in range(DT)] + [sv[s] for s in range(NSUB)]
                    + [kc[p] for p in range(NP)] + [vc[p] for p in range(NP)])
        ...
        def _unpack(st):
            ...
            return m_run, l_run, o_acc, sv, kc, vc
        ...
        def loop_body(kt_iv, m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2, do_mask):
            ...
            kdv16_all = load_descale(kv0_outer, do_mask)  # issue descale global loads EARLY (hidden by GEMM1)
            fx.rocdl.s_setprio(1)
            # GEMM1(tile i+1) MFMAs emitted right before softmax(tile i) VALU -> scheduler overlaps
            # the MFMA burst with the VALU-bound softmax (the ~13:1 wall) AND the descale loads above.
            sv_next = gemm1_tile(kbuf_n1)
            m_run, l_run, o_acc = softmax_gemm2_tile(kv0_outer, vbuf, sv_cur, kdv16_all, m_run, l_run, o_acc, do_mask)
            ...
            if const_expr(BUFK):
                _wait_vmem()
            fx.gpu.barrier()
            return m_run, l_run, o_acc, sv_next, kc_np3, vc_np3
        ...
        init_state = _pack(m_run0, l_run0, o_acc0, sv0, kc_w2, vc_w2)
        for kt_iv, st in range(fx.Index(0), fx.Index(n_unmask), fx.Index(1), init=init_state):
            m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2 = _unpack(st)
            m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2 = loop_body(kt_iv, m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2, False)
            st = yield _pack(m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2)

        # Phase 2: masked tiles (diagonal + any OOB tail). Carries sv + prefetch across the split.
        mid_state = st
        for kt_iv, st in range(fx.Index(n_unmask), fx.Index(n_kt_rt), fx.Index(1), init=mid_state):
            m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2 = _unpack(st)
            m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2 = loop_body(kt_iv, m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2, True)
            st = yield _pack(m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2)

        m_run, l_run, o_acc, _sv, _kc, _vc = _unpack(st)
