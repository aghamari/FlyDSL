...
"""FP8 causal FMHA prefill — layoutmax + packed score-scale + wait-batch + CROSS-TILE PIPELINE.

Lineage: ``layoutmax`` -> ``_valu`` (packed score-scale) -> ``_valupt`` (P-transpose wait-batch)
-> THIS (+ cross-tile GEMM1 software pipeline). Three stacked levers over the maximally
layout-algebraic hk5:
  ...
  3. PIPELINE (runbook §8.1, autoresearch lever #1): ``compute_kt_tile`` split into ``gemm1_tile``
     ...
     tile i+1's GEMM1 MFMAs IMMEDIATELY BEFORE tile i's softmax VALU, so the MFMA unit fills during
     the VALU-bound (~13:1) softmax = in-wave MFMA/VALU overlap. Needs TRIPLE-buffered LDS (NBUF=3:
     tile i's V + tile i+1's K + prefetched tile i+2 all live); +26 VGPR (211->237, still 2 waves).

MEASURED (MI308X gfx942, isolated+repeated medians) vs base ``layoutmax``:
  sq2048   26 -> 27 TF      sq16384  382 -> 408 TF (+6.8%)      sq32768  359 -> 438 TF (+22%)
vs previous best ``_valupt``: +5.4% / +5.0% at sq16384 / sq32768. Correctness byte-identical to hk5
on all shapes (causal/non-causal/GQA/batch). This is the current best of the layoutmax lineage.
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_layoutmax_pipe_smem")
...
def attn_kernel(
    ...
    def process_qtile(qtile):
        ...
        def softmax_gemm2_tile(kv0_outer, vbuf, sv, m_run, l_run, o_acc, do_mask):
            # --- descale + causal mask for all subtiles -> s_vals[sub][i] ---
            # VALU LEVER (runbook §7.4/§8.5): the per-kv descale scaling S = sv * qs * kdv runs on
            # EVERY tile. Do it as PACKED vector muls (16-wide sv * (kdv16 * qs)) instead of 16 scalar
            # f32 muls per subtile, so it folds into v_pk_mul_f32. Causal mask (diagonal tiles) stays
            # scalar. Measured +9% @ sq32768 on the hk5 lineage; correctness byte-identical.
            ...
            for sub in fx.range_constexpr(NSUB):
                ...
                kdv16 = fx.Vector.from_elements([kdv[g][j] for g in range(4) for j in range(4)], fx.Float32)
                s16 = fx.Vector(sv[sub]) * (fx.Vector(kdv16) * qs_vec)
                s_sub = []
        ...
        def loop_body(kt_iv, m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2, do_mask):
            ...
            # the MFMA burst with the VALU-bound softmax (the ~13:1 wall).
            sv_next = gemm1_tile(kbuf_n1)
            m_run, l_run, o_acc = softmax_gemm2_tile(kv0_outer, vbuf, sv_cur, m_run, l_run, o_acc, do_mask)
