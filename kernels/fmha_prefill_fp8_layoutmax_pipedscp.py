...
"""FP8 causal FMHA prefill — layoutmax + score-scale + wait-batch + PIPELINE + descale PREFETCH.

Lineage: ``layoutmax`` -> ``_valu`` -> ``_valupt`` -> ``_pipe`` -> ``_pipedsc`` (descale hoist)
-> THIS (``_pipedscp``: descale PREFETCHED one tile ahead, carried in loop state, so it is fully
off the score critical path; the mask-clamp is dropped since buffer loads are bounds-checked and
OOB kv are masked to -inf anyway). Marginal win over ``_pipedsc`` (sq16384 410->415, sq32768
441->442) at +22 VGPR (216->238, still 2 waves) — keep ``_pipedsc`` as the leaner fallback if a
later lever needs the VGPR headroom. Five stacked levers over the maximally layout-algebraic hk5:
  1. VALU (runbook §7.4/§8.5): per-kv descale ``S = sv*qs*kdv`` as PACKED vector muls (``v_pk_mul_f32``).
  2. WAIT (runbook §8.3, ATT-driven): all 8 ``ds_bpermute`` of a subtile before ONE ``lgkmcnt(0)``.
  3. PIPELINE (runbook §8.1, autoresearch lever #1): ``compute_kt_tile`` split into ``gemm1_tile``
     (K@Q^T MFMAs) + ``softmax_gemm2_tile``; the loop carries ``sv`` = GEMM1(current tile) and emits
     tile i+1's GEMM1 MFMAs IMMEDIATELY BEFORE tile i's softmax VALU = in-wave MFMA/VALU overlap.
     Needs TRIPLE-buffered LDS (NBUF=3: tile i's V + tile i+1's K + prefetched tile i+2 all live).
  4. DESCALE-DECOUPLE (runbook §4.6/§8.1, ATT: residual vmcnt in compute): the per-kv K-descale
     global load is issued at the TOP of loop_body (before the pipelined GEMM1) so its latency hides
     behind the MFMAs, off the score critical path. Also drops VGPR 237->216.

MEASURED (MI308X gfx942, isolated+repeated medians) vs base ``layoutmax``:
  sq2048  26 -> 27 TF     sq16384  382 -> 410 TF (+7.3%)     sq32768  359 -> 441 TF (+22.8%)
vs ``_pipe``: +0.5% / +0.7% and VGPR 237->216. Correctness byte-identical to hk5 on all shapes
(causal/non-causal/GQA/batch). This is the current best of the layoutmax lineage.

--- base kernel below ---
FP8 causal FMHA prefill (paged, vec_k_col_v) — MAXIMALLY LAYOUT-ALGEBRAIC variant of hk5.

Identical algorithm/tiling/perf-knobs to ``fmha_prefill_fp8_ck_hk5``, but BOTH matmuls go through
the layout-algebra MMA path: one typed ``make_mma_atom(MFMA 32x32x16 fp8)`` -> ``make_tiled_mma``
-> per-lane ``thr_slice`` whose derived A/B/C fragments drive ``fx.gemm`` (no raw
``mfma_f32_32x32x16_fp8_fp8`` calls, no manual i64 operand packing). Q/K/V/P operands are fed as
typed ``make_fragment_A/B/C`` tiles.
...
NSUB = KT // BN
NBUF = int(os.environ.get("FMHA_NBUF", "3"))  # TRIPLE-buffer: GEMM1-pipelined loop keeps tile i (V),
# tile i+1 (K, for pipelined GEMM1) AND stores tile i+2 -> 3 live LDS tiles.
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_layoutmax_pipedscp_smem")
...
def attn_kernel(
    ...
    def process_qtile(qtile):
        ...
        # GEMM1 half (S = K@Q^T) split out so the pipeline can issue tile i+1's MFMAs next to tile i's
        # softmax VALU (cross-tile independent-MFMA overlap; runbook §8.1, autoresearch lever #1).
        def gemm1_tile(kbuf):
            sv = []
        ...
        # DESCALE-DECOUPLE LEVER (runbook §4.6/§8.1, ATT: residual vmcnt in compute). The per-kv K
        # descale is a GLOBAL load needed immediately for the score scaling. Issue it at the TOP of
        # loop_body (before the pipelined GEMM1) so the GEMM1 MFMAs hide its latency, off the score
        # critical path. Returns the per-subtile 16-wide descale vector (already gathered).
        # Unclamped: buffer_load is bounds-checked (OOB kv -> 0), and any OOB kv (>=sk) is masked to
        # -inf in softmax anyway, so no clamp is needed for correctness -> also drops the mask-clamp
        # VALU. This lets the descale be PREFETCHED one tile ahead without knowing the tile's phase.
        def load_descale(kv0_outer):
            kdv16_all = []
            for sub in fx.range_constexpr(NSUB):
                ...
                kdv16_all.append(fx.Vector.from_elements([kdv[g][j] for g in range(4) for j in range(4)], fx.Float32))
            return kdv16_all

        def softmax_gemm2_tile(kv0_outer, vbuf, sv, kdv16_all, m_run, l_run, o_acc, do_mask):
            # --- descale + causal mask for all subtiles -> s_vals[sub][i] ---
            # VALU LEVER (runbook §7.4/§8.5): packed score-scale S = sv * (kdv16 * qs) (v_pk_mul_f32);
            # kdv16 was preloaded at the top of loop_body (descale-decouple). Mask stays scalar.
            qs = q_descale * fx.Float32(sm_scale * LOG2E)  # per-lane const (folds sm AND log2e into descale)
            qs_vec = fx.Vector.filled(16, fx.Float32(qs), fx.Float32)
            s_all = []  # flat list over subtiles
            for sub in fx.range_constexpr(NSUB):
                kv0 = kv0_outer + fx.Int32(sub * BN)
                s16 = fx.Vector(sv[sub]) * (fx.Vector(kdv16_all[sub]) * qs_vec)
                s_sub = []
            ...
            # WAIT LEVER (runbook §8.3, ATT: lgkmcnt LDS-wait ~17% of stall): issue ALL ds_bpermute
            # for the subtile (both s halves = 8 permutes) BEFORE a SINGLE _wait_lds, instead of one
            # lgkmcnt(0) wait per s half. More permutes in flight, half the LDS waits on the P path.
            p_i64_all = []
            for sub in fx.range_constexpr(NSUB):
                p_vals = p_all[sub]
                bperm = []
                for s in fx.range_constexpr(2):
                    ...
                    bperm.append((h0_b0, h0_b1, h1_b0, h1_b1))
                _wait_lds()  # single wait for all 8 permutes of this subtile
                p_i64_s = []
                for s in fx.range_constexpr(2):
                    h0_b0, h0_b1, h1_b0, h1_b1 = bperm[s]
        ...
        # CROSS-TILE PIPELINE (runbook §8.1, autoresearch lever #1 — hide softmax VALU behind MFMA).
        # Carry sv = GEMM1(current tile) in loop state. Each iter: store tile i+1 to LDS + barrier,
        # then emit GEMM1(tile i+1) MFMAs IMMEDIATELY BEFORE softmax/GEMM2(tile i) VALU, so the MFMA
        # unit fills during the (VALU-bound, ~13:1) softmax = in-wave MFMA/VALU overlap. Ping-pong LDS
        # keeps both tile i (V for GEMM2) and tile i+1 (K for GEMM1) resident.
        # State: [m, l, o_acc(DT), sv(NSUB), kc(NPASS), vc(NPASS)]; kc/vc = tile i+1 loaded regs.
        NP = const_expr(NPASS)

        def _pack(m_run, l_run, o_acc, sv, kc, vc, kdv):
            return ([m_run, l_run] + [o_acc[d] for d in range(DT)] + [sv[s] for s in range(NSUB)]
                    + [kc[p] for p in range(NP)] + [vc[p] for p in range(NP)] + [kdv[s] for s in range(NSUB)])
        ...
        def _unpack(st):
            ...
            b = 2 + DT
            sv = [st[b + s] for s in range(NSUB)]
            b2 = b + NSUB
            kc = [st[b2 + p] for p in range(NP)]
            vc = [st[b2 + NP + p] for p in range(NP)]
            b3 = b2 + 2 * NP
            kdv = [st[b3 + s] for s in range(NSUB)]
            return m_run, l_run, o_acc, sv, kc, vc, kdv

        # Triple-buffered: at iter i, LDS[i%3]=tile i (V for GEMM2), LDS[(i+1)%3]=tile i+1 (K for the
        # pipelined GEMM1), and we store the prefetched tile i+2 into the FREE buffer LDS[(i+2)%3].
        # Carried regs kc/vc hold tile i+2 (to store this iter). One barrier/iter (store->free buffer).
        def loop_body(kt_iv, m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2, kdv_cur, do_mask):
            kv0_outer = fx.Int32(kt_iv) * fx.Int32(KT)
            cur_buf = fx.Int32(kt_iv) % fx.Int32(3)
            nxt1_buf = (fx.Int32(kt_iv) + fx.Int32(1)) % fx.Int32(3)
            nxt2_buf = (fx.Int32(kt_iv) + fx.Int32(2)) % fx.Int32(3)
            vbuf = cur_buf * fx.Int32(_V_BYTES)
            kbuf_n1 = nxt1_buf * fx.Int32(_K_BYTES)
            kbuf_n2 = nxt2_buf * fx.Int32(_K_BYTES)
            vbuf_n2 = nxt2_buf * fx.Int32(_V_BYTES)
            kdv_next = load_descale(kv0_outer + fx.Int32(KT))  # PREFETCH tile i+1 descale (1 iter ahead)
            fx.rocdl.s_setprio(1)
            # GEMM1(tile i+1) MFMAs emitted right before softmax(tile i) VALU -> scheduler overlaps
            # the MFMA burst with the VALU-bound softmax (the ~13:1 wall). Descale for tile i is already
            # resident (prefetched last iter) so it's fully off the score critical path.
            sv_next = gemm1_tile(kbuf_n1)
            m_run, l_run, o_acc = softmax_gemm2_tile(kv0_outer, vbuf, sv_cur, kdv_cur, m_run, l_run, o_acc, do_mask)
            fx.rocdl.s_setprio(0)
            # store prefetched tile i+2 into the free buffer, prefetch tile i+3 for next iter.
            store_kv_to_lds(kc_np2, vc_np2, kbuf_n2, vbuf_n2)
            kc_np3, vc_np3 = load_kv_regs(kv0_outer + fx.Int32(3 * KT))
            if const_expr(BUFK):
                _wait_vmem()
            fx.gpu.barrier()
            return m_run, l_run, o_acc, sv_next, kc_np3, vc_np3, kdv_next

        # Prologue: stage tiles 0 and 1 into LDS0/LDS1, compute GEMM1(tile 0), prefetch tile 2 regs
        # and tile 0's descale.
        kc_w0, vc_w0 = load_kv_regs(fx.Int32(0))
        store_kv_to_lds(kc_w0, vc_w0, fx.Int32(0), fx.Int32(0))
        kc_w1, vc_w1 = load_kv_regs(fx.Int32(KT))
        store_kv_to_lds(kc_w1, vc_w1, fx.Int32(_K_BYTES), fx.Int32(_V_BYTES))
        if const_expr(BUFK):
            _wait_vmem()
        fx.gpu.barrier()
        sv0 = gemm1_tile(fx.Int32(0))
        kc_w2, vc_w2 = load_kv_regs(fx.Int32(2 * KT))
        kdv0 = load_descale(fx.Int32(0))

        # Phase 1: unmasked interior tiles (no per-element mask VALU).
        init_state = _pack(m_run0, l_run0, o_acc0, sv0, kc_w2, vc_w2, kdv0)
        for kt_iv, st in range(fx.Index(0), fx.Index(n_unmask), fx.Index(1), init=init_state):
            m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2, kdv_cur = _unpack(st)
            m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2, kdv_cur = loop_body(kt_iv, m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2, kdv_cur, False)
            st = yield _pack(m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2, kdv_cur)

        # Phase 2: masked tiles (diagonal + any OOB tail). Carries sv + prefetch + descale across split.
        mid_state = st
        for kt_iv, st in range(fx.Index(n_unmask), fx.Index(n_kt_rt), fx.Index(1), init=mid_state):
            m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2, kdv_cur = _unpack(st)
            m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2, kdv_cur = loop_body(kt_iv, m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2, kdv_cur, True)
            st = yield _pack(m_run, l_run, o_acc, sv_cur, kc_np2, vc_np2, kdv_cur)

        m_run, l_run, o_acc, _sv, _kc, _vc, _kdv = _unpack(st)
