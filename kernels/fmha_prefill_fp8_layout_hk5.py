...
"""FP8 causal FMHA prefill (paged, vec_k_col_v) for gfx942 — LAYOUT-API rewrite of hk5.

★ IDENTICAL to the canonical ``fmha_prefill_fp8_ck_hk5.py`` (same body, features, LDS padding,
column-V, diagonal-pair, softmax) EXCEPT the two GEMMs are expressed through the FlyDSL MMA
*atom* layout API — ``fx.make_mma_atom(fx.rocdl.MFMA(32,32,16, fp8))`` + ``fly.mma_atom_call_ssa``
— instead of the raw ``fx.rocdl.mfma_f32_32x32x16_fp8_fp8`` intrinsic. The MFMA shape/dtype is
named ONCE as a typed atom and reused at both call sites (matches flash_attn_gfx950.py).

★ WHY ONLY THE GEMM ATOM (and not "more" layout algebra) — this kernel is VGPR/occupancy-bound
(165 VGPR @ 3 waves/SIMD; the 4th wave needs <=128, unreachable in the 0.2.0 wheel). Layout-algebra
address abstractions COMPUTE ADDRESSES IN REGISTERS, so they can only be VGPR-neutral (readability)
or REGRESS — they cannot buy occupancy. The other layout pushes are documented dead-ends:
  * ``fx.gemm`` / ``make_tiled_mma`` / ``make_fragment_A/B/C`` CANNOT express the chained
    attention GEMMs: GEMM1's C-output lane layout != GEMM2's B-operand layout, and the bespoke
    ds_bpermute P-transpose is the fix for that exact mismatch (flash_attn_gfx950.py concurs).
  * K/V is a PAGED jagged gather (page-table lookup via LTD/LTP), which a ``make_view`` layout
    view cannot index — it stays on ``create_buffer_resource`` + ``buffer_load``.
  * The K/V LDS tiles COULD be ``make_view`` layout views, but the addressing is VGPR-neutral at
    best on this occupancy-locked kernel, so it is not done here (the SmemPtr path is kept).
This file is therefore the maximal layout-API form that preserves hk5's measured performance,
built on hk5 (the large-seq winner) rather than log2dom (what ``fmha_prefill_fp8_layout.py`` used,
which regresses at large seq). See the flydsl-fmha-prefill-opt skill for the full ledger.
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_layout_hk5_smem")
