...
"""FP8 causal FMHA prefill — layoutmax + vectorized score-scaling + P-transpose wait-batching.

Lineage: ``layoutmax`` -> ``layoutmax_valu`` (packed score-scale) -> THIS (+ P-transpose LDS
wait-batching). Two stacked levers over the maximally layout-algebraic hk5:
  1. VALU (runbook §7.4/§8.5): per-kv descale ``S = sv*qs*kdv`` as PACKED vector muls (``v_pk_mul_f32``)
     instead of 16 scalar f32 muls/subtile.
  2. WAIT (runbook §8.3, ATT-driven): the P-transpose issues all 8 ``ds_bpermute`` of a subtile
     before a SINGLE ``s_waitcnt lgkmcnt(0)``, instead of one wait per s-half — cuts the LDS-wait
     stalls (ATT showed lgkmcnt ~17% of stall) and tightens run-to-run variance.
Measured (MI308X, isolated+repeated medians): sq16384 382->387 TF, sq32768 359->417 TF (+16%),
32768 timing very stable (417/418/417). Correctness byte-identical to hk5 on all shapes.
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_layoutmax_valupt_smem")
