...
"""FP8 causal FMHA prefill — hk5 + VECTORIZED score-scaling (runbook §7.4/§8.5 VALU lever).

Identical to ``fmha_prefill_fp8_ck_hk5`` except the per-kv descale score-scaling
``S = sv * qs * kdv`` (run on EVERY kv tile) is done as PACKED vector muls (16-wide
``sv * (kdv16 * qs)``) instead of 16 scalar f32 muls per subtile, so it folds into
``v_pk_mul_f32``. The causal mask (diagonal tiles only) stays scalar.

MEASURED (MI308X gfx942, bs1 nq8 nk1 causal, isolated+repeated medians):
  sq32768  382 -> 416 TF  (+9%, non-overlapping across 3 runs)   ← the win
  sq16384  386 -> 390 TF  (tie, but markedly more run-to-run stable)
  sq1024     8 ->   8 TF  (tie; valu rock-stable vs hk5's cold-run swing)
Correctness byte-identical to hk5 (same ERR on every shape, incl. the pre-existing
pscale<1 fp8-tolerance FAIL that hk5 also hits). Costs +46 VGPR (165->211, 3->2
waves/SIMD) but the kernel is VALU/latency-bound at large seq so occupancy loss is
hidden and the packed-mul ILP nets a win. See ``fmha_prefill_fp8_ck_hk5`` for the base
design; perf history + dead-end ledger in ``learn_fmha/docs/JOURNAL.md`` §2–3.
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_valu_smem")
...
def attn_kernel(
    ...
    def process_qtile(qtile):
        ...
        def compute_kt_tile(kv0_outer, kbuf, vbuf, m_run, l_run, o_acc, do_mask):
            ...
            # VALU LEVER (#2): the per-kv descale scaling S = sv * qs * kdv runs on EVERY tile.
            # Do it as PACKED vector muls (16-wide sv * (16-wide kdv * qs)) instead of 16 scalar
            # f32 muls per subtile, so the score-scale can fold into v_pk_mul_f32. The causal mask
            # (diagonal tiles only) stays scalar.
