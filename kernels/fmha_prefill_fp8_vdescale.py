...
Variant of ``fmha_prefill_fp8_ck_hk5`` with ONE change (lever F2): the per-element score
descale in ``compute_kt_tile`` is VECTORIZED. hk5 does ~16 scalar f32 multiplies per subtile
(``s = sv[sub][i] * (qs * kdv[i//4][i%4])``); here we assemble a 16-wide ``scale_vec`` from the
K-descale f32x4's (broadcasting the per-lane ``qs``) and compute ``s_vec = fx.Vector(sv[sub]) *
scale_vec`` once, so the multiplies lower to packed ``v_pk_mul_f32`` (2 lanes/instr) — mirroring
the existing ``fx.Vector(o_acc[dt]) * corr_vec`` rescale. Pure VALU-count reduction; the causal
mask + max + exp downstream are bit-identical. Everything else matches hk5 exactly.

★ Based on canonical FlyDSL FMHA kernel. Full design notes, perf history, the VALU-fold/COLUMN-V
wins, occupancy diagnosis, and the dead-end ledger live in ``learn_fmha/docs/JOURNAL.md`` §2–3.
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_vdescale_smem")
...
def attn_kernel(
    ...
    def process_qtile(qtile):
        ...
        def compute_kt_tile(kv0_outer, kbuf, vbuf, m_run, l_run, o_acc, do_mask):
            ...
            # F2: VECTORIZED descale. hk5 did `s = sv[sub][i] * (qs * kdv[i//4][i%4])` as 16 SCALAR
            # f32 multiply pairs per subtile. Here we assemble a 16-wide `scale_vec[i] = qs *
            # kdv[i//4][i%4]` from the K-descale f32x4's (broadcasting the per-lane scalar `qs`) and
            # do the descale as two packed 16-wide multiplies (`v_pk_mul_f32`, 2 lanes/instr) — the
            # same idiom as the o_acc `corr_vec` rescale below. fp multiply is commutative bit-exact,
            # and the lane->element mapping (i -> group i//4, elem i%4) is preserved, so s_vec[i] ==
            # the old scalar `s` BIT-FOR-BIT; the mask select + max + exp downstream are unchanged.
            qs = q_descale * fx.Float32(sm_scale * LOG2E)  # per-lane const (folds sm AND log2e into descale)
            qs_vec = fx.Vector.filled(16, qs, fx.Float32)
            ...
            for sub in fx.range_constexpr(NSUB):
                ...
                # Pack the four K-descale f32x4's into one 16-wide vector ordered i = g*4 + e (== the
                # scalar path's kdv[i//4][i%4]), fold in qs, then descale all 16 scores at once.
                kd_vec = fx.Vector.from_elements([kdv[g][e] for g in range(4) for e in range(4)], fx.Float32)
                scale_vec = fx.Vector(kd_vec) * qs_vec
                s_vec = fx.Vector(sv[sub]) * scale_vec
                s_sub = []
