...
"""FP8 causal FMHA prefill — SMALL-SEQLEN-tuned variant (1k-8k) of the clean layout-algebra kernel.

★ Same code as fmha_prefill_fp8_layout_clean.py, but carries a device-fair-swept per-shape config
dispatcher (`get_kernel(sq)` / `best_config(sq)`) tuned for sq 1024-8192. The full NWAVES×KT×DIAG
sweep (see the dispatch block at the bottom) found NWAVES=4 optimal everywhere and per-seqlen
(KT,DIAG): 1024->KT64/DIAG0, 2048->KT64/DIAG1, 4096->KT32/DIAG1, 8192->KT32/DIAG0. Use this file as
the place to specialize small-seq optimizations. Measurement methodology:
``flydsl-kernel-bench-discipline`` skill; bench via tests/kernels/bench_fmha_fair.py (device-fair).
...
def attn_kernel(
    ...
    def process_qtile(qtile):
        ...
        def compute_kt_tile(kv0_outer, kbuf, vbuf, m_run, l_run, o_acc, do_mask):
            ...
            # qs (per-lane score scale) is hoisted to process_qtile; half4 to the kernel top.
            s_all = []  # flat list over subtiles
            for sub in fx.range_constexpr(NSUB):
                kv0 = kv0_outer + fx.Int32(sub * BN)
                kv_lane = kv0 + half4   # per-(subtile,lane) base; the +g*8/+const(i) below are immediates
                kdv = []
                ...
                for i in fx.range_constexpr(16):
                    ...
                    if const_expr(do_mask):
                        kv = kv_lane + fx.Int32((i // 4) * 8 + (i % 4))   # const(i) folds to one immediate
