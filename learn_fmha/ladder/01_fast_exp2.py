...
"""Ladder 01 — FAST EXP2 (Lesson 13): rocdl.exp2 instead of the generic Float32.exp2().

ONE change vs 00_baseline: the softmax exp uses the hardware `rocdl.exp2` (1 VALU op)
instead of `Float32.exp2()` (which also emits a v_ldexp range-reduction pair, ~3 VALU).
Math is identical (exp(x)=2^(x*log2e); softmax always evaluates exp(s-m) with s-m<=0, so
the fast path is range-safe). A pure VALU micro-optimization: roughly neutral on this
occupancy-starved single-wave shape, but it removes 2 VALU ops per exp in a kernel that is
VALU-heavy (~23:1 VALU:MFMA) once scaled up.

Everything else is identical to 00_baseline (layout-algebra GEMMs via fx.gemm; LDS
P-transpose; row-major V byte-gather).

Run one shape:  HIP_VISIBLE_DEVICES=2 python3 learn_fmha/ladder/01_fast_exp2.py 256 256 1
All shapes:     HIP_VISIBLE_DEVICES=2 python3 learn_fmha/ladder/01_fast_exp2.py
"""
import os
import sys
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="ladder01_smem")
...
def _run_one(sq, sk, causal):
    ...
    ok = _bench.check(f"01_fast_exp2 sq={sq} sk={sk} causal={causal}", O, ref)
