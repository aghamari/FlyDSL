...
"""Ladder 02 — CAUSAL KV-BOUND (Lesson 14): stop the kv loop at the causal limit.

ONE change vs 01_fast_exp2: with causal masking, a q-tile at rows [blk*BM, blk*BM+BM) can
only attend to kv <= (blk*BM+BM-1) + (sk-sq). So instead of looping all ceil(sk/BN) kv-tiles
and masking the tail to -inf, we cap the loop count `n_kv` at the last tile that contains any
valid kv. This cuts ~half the kv work on the causal triangle.

Caveat (Lesson 14's real point): cutting *work* does not help an *under-occupied* machine —
on this single-wave shape the freed cycles are not the bottleneck, so wall time barely moves.
It becomes a real win once the grid fills the CUs (structural tier).

Everything else is identical to 01_fast_exp2.

Run one shape:  HIP_VISIBLE_DEVICES=2 python3 learn_fmha/ladder/02_causal_bound.py 256 256 1
All shapes:     HIP_VISIBLE_DEVICES=2 python3 learn_fmha/ladder/02_causal_bound.py
"""
import os
import sys
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="ladder02_smem")
...
def _run_one(sq, sk, causal):
    ...
    ok = _bench.check(f"02_causal_bound sq={sq} sk={sk} causal={causal}", O, ref)
