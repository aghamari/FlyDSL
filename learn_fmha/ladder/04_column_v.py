...
"""Ladder 04 — COLUMN-MAJOR V (Lesson 17): delete the V transpose (the biggest win).

ONE change vs 03_register_p: V is stored COLUMN-major `[d, kv]` in global memory (the KV-cache
packer does this once). GEMM2 contracts over kv, so its V operand — 8 contiguous kv for a fixed
d — is now a single WIDE load instead of the 8-byte row-major gather. The transpose isn't moved
(rung 03) — it is DELETED, because the layout makes the contraction axis already contiguous.

This is the punchline of the tutorial's Part D: when a cost is irreducible in the current layout
(the V/P transpose), change the layout so the op disappears. On gfx942 this beats every
transpose trick (LDS scatter, ds_bpermute, v_perm).

Assumes sk is a multiple of BN=32 (full kv-tiles) so the wide column-V load never crosses a d-row
boundary; the ladder's shapes satisfy this. Everything else is identical to 03_register_p.

Run one shape:  HIP_VISIBLE_DEVICES=2 python3 learn_fmha/ladder/04_column_v.py 256 256 1
All shapes:     HIP_VISIBLE_DEVICES=2 python3 learn_fmha/ladder/04_column_v.py
"""
import os
import sys
...
def _run_one(sq, sk, causal):
    ...
    ok = _bench.check(f"04_column_v sq={sq} sk={sk} causal={causal}", O, ref)
