...
"""Ladder 06 — COOPERATIVE K/V -> LDS (Lesson 10): waves share one staged tile.

ONE change vs 05_multiwave: the NWAVES waves in a workgroup all attend the SAME kv range, so
loading K/V per-wave from global is redundant. Here the workgroup COOPERATIVELY loads the K
tile [kv,hd] and V tile [d,kv] into LDS ONCE per kv-tile (all NWAVES*64 threads participate),
`barrier`, then every wave reads the shared tile from LDS. This removes the redundant global
traffic; the cost is a workgroup barrier per kv-tile (Lesson 10 -> only fully pays off pipelined).

Q stays per-wave (each wave has its own q-rows). GEMM1 now reads K from LDS; GEMM2 reads the
column-major V from LDS. This is the full structural kernel (multiwave + column-V + register-P +
cooperative-LDS) — the layout-algebra sibling of lesson_22b, with fx.gemm driving the MFMAs.

Run one shape:  HIP_VISIBLE_DEVICES=2 python3 learn_fmha/ladder/06_cooperative_lds.py 1024 1024 1
All shapes:     HIP_VISIBLE_DEVICES=2 python3 learn_fmha/ladder/06_cooperative_lds.py
"""
import os
import sys
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="ladder06_smem")
...
def attn_kernel(Q: fx.Tensor, K: fx.Tensor, V: fx.Tensor, O: fx.Tensor,
    ...
    for kt, st in range(fx.Index(0), fx.Index(n_kv), fx.Index(1), init=init):
        ...
        for p in fx.range_constexpr((NSLOT_V + NTHREADS - 1) // NTHREADS):
            ...
            voff = d_row * sk_i + gkv_s        # column-major V[d, kv]
...
def _run_one(sq, sk, causal):
    ...
    ok = _bench.check(f"06_cooperative_lds sq={sq} sk={sk} causal={causal}", O, ref)
