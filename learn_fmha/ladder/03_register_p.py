...
"""Ladder 03 — REGISTER-RESIDENT P (Lesson 12): ds_bpermute instead of the LDS P round-trip.

ONE change vs 02_causal_bound: the P-transpose (P is produced as S[kv,q] but GEMM2 needs it
as P[q,kv]) is now done ACROSS LANES with `ds_bpermute` (a lane reads another lane's register),
instead of storing P to LDS, barrier, and reloading. This removes the P-LDS store/reload and
its barrier -> this kernel uses NO LDS at all.

The decisive lesson (Lesson 12): `ds_bpermute` is itself an LDS-unit op, so register-P *moves*
the transpose traffic rather than deleting it. It is the setup for rung 04 (column-V), which
DELETES the transpose entirely by changing V's layout.

Everything else is identical to 02_causal_bound. The ds_bpermute pattern (2 kv sub-groups x
2 packed dwords x 2 half destinations) is the verified 32x32x16 pattern from lesson_22b.

Run one shape:  HIP_VISIBLE_DEVICES=2 python3 learn_fmha/ladder/03_register_p.py 256 256 1
All shapes:     HIP_VISIBLE_DEVICES=2 python3 learn_fmha/ladder/03_register_p.py
"""
import os
import sys
...
def _run_one(sq, sk, causal):
    ...
    ok = _bench.check(f"03_register_p sq={sq} sk={sk} causal={causal}", O, ref)
